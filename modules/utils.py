...
def cli():
    argparser = ArgumentParser(
        description="keyobell | A project for scanning "
    ...
    reportargs.add_argument(
        "-o",
        "--output",
        help="Output file name. (Default : keyobell.log)",
        default="keyobell",
        type=str,
        required=False,
    )
...
def InitArgsConf(args, log) -> None:
    ...
    try:
        ...
        if config.has_option("keyobell", "target"):
            args.target = config.get("keyobell", "target").lower()

        if config.has_option("keyobell", "hostfile"):
            args.host_file = config.get("keyobell", "hostfile").lower()

        if config.has_option("keyobell", "scantype"):
            args.scan_type = config.get("keyobell", "scantype").lower()

        if config.has_option("keyobell", "nmapflags"):
            args.nmap_flags = config.get("keyobell", "nmapflags").lower()

        if config.has_option("keyobell", "speed"):
            try:
                args.speed = int(config.get("keyobell", "speed"))
            except ValueError:
                log.logger("error", "Invalid speed value in config file. (Default : 3)")

        if config.has_option("keyobell", "apikey"):
            args.api = config.get("keyobell", "apikey").lower()

        if config.has_option("keyobell", "auto"):
            args.yes_please = True

        if config.has_option("keyobell", "mode"):
            args.mode = config.get("keyobell", "mode").lower()

        if config.has_option("keyobell", "noisetimeout"):
            args.noise_timeout = config.get("keyobell", "noisetimeout").lower()
...
def check_version(cur_version: str, log) -> None:
    try:
        ...
        if version_major > cur_version_major:
            log.logger(
                "warning",
                "Your version of keyobell is outdated. Update is advised.",
            )
